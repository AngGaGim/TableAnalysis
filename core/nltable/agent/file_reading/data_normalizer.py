...
from core.nltable.errors import SimpleOutputParserException
...
5. Ensure that the table headers are in the first sublist, followed by each data row in subsequent sublists.
6. Provide only the restructured table in the required format, without any Python code or unnecessary text.
7. The output format should be [[], [], []], not [[][][]], meaning each sublist represents a row of data.
Input:{table}
Output:"""
...
# Take the df rows and create a new DataFrame as `final_df`
