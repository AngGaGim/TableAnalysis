...
from core.nltable.agent.output_parser import MarkdownOutputParser
from core.nltable.retriever import format_columns
from core.nltable.safety import create_hazard_classifier
from core.nltable.tools import (
    IPythonTool,
    markdown_console_template,
    process_content,
)

from core.nltable.utils import filter_contents
from pathlib import Path
from langchain_core.agents import AgentAction, AgentFinish
from langchain_core.language_models import BaseLanguageModel
from langchain_core.retrievers import BaseRetriever
from langchain_core.runnables import Runnable
from core.nltable.pybox.base import BasePyBoxManager


INSTRUCTION = """You are an expert Python data analyst. Your job is to help user analyze datasets by writing Python code. Each markdown codeblock you write will be executed in an IPython environment, and you will receive the execution output. You should provide results analysis based on the execution output.
    ...
    *,
    ...
    def input_guard(
        ...
        if hazard_classifier is not None:
            ...
            flag, category = hazard_classifier.invoke(input={"input": last_message.content})
            if flag == "unsafe" and category is not None:
                last_message.additional_kwargs["hazard"] = category
                return {"messages": [last_message]}
        return {"messages": []}

    def retrieve_columns(state: AgentState) -> dict:
        if dataset_retriever is None:
            return {"messages": []}

        last_message = state["messages"][-1]
        docs = dataset_retriever.invoke(
    ...
    def vlm_agent_node(state: AgentState) -> dict:
        # Truncate messages based on message count.
        ...
        agent_outcome: AgentAction | AgentFinish = vlm_agent.invoke(
            {
                "messages": temp_messages,
                "date": state["date"],
            }
    ...
    def tool_node(state: AgentState) -> dict:
        messages: list[ToolMessage] = tool_executor.invoke(state["messages"])
